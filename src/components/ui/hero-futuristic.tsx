...
export function HeroFuturistic({
  ...
  return (
    ...
    >
      {/* #020814/#030712 are one-off near-black depth stops darker than --color-brand-navy, not reusable tokens */}
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_40%,color-mix(in_oklab,var(--color-brand-gold)_18%,transparent),transparent_25%),radial-gradient(circle_at_50%_50%,color-mix(in_oklab,var(--color-brand-navy-light)_80%,transparent),transparent_42%),linear-gradient(180deg,#020814_0%,var(--color-brand-navy)_48%,#030712_100%)]" />
      ...
      <div className="pointer-events-none absolute left-1/2 top-1/2 z-0 h-[19rem] w-[19rem] -translate-x-1/2 -translate-y-[58%] opacity-50 sm:h-[26rem] sm:w-[26rem] sm:opacity-60 lg:left-[56%] lg:h-[33rem] lg:w-[33rem]">
        <Canvas camera={{ position: [0, 0, 5.2], fov: 42 }} dpr={[1, 1.5]}>
          ...
          <pointLight position={[2.8, 2.4, 3]} intensity={5.8} color={brandColors.goldLight} />
      ...
      <div className="pointer-events-none absolute left-1/2 top-1/2 z-0 h-[13rem] w-[46rem] -translate-x-1/2 translate-y-12 bg-[radial-gradient(ellipse_at_center,color-mix(in_oklab,var(--color-brand-gold)_10%,transparent),transparent_64%)] blur-2xl" />
